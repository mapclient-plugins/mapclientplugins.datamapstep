...
from cmlibs.maths.vectorops import add, cross, div, magnitude, mult, sub
from cmlibs.utils.zinc.general import ChangeManager
from cmlibs.widgets.sceneviewerwidget import SceneviewerWidget, SelectionMode
...
class NodeEditorSceneviewerWidget(SceneviewerWidget):
    ...
    def projectLocal(self, x, y, z, localScene):
        ...
        """
        in_coords = [x, y, z]
        result, out_coords = self._sceneviewer.transformCoordinates(SCENECOORDINATESYSTEM_LOCAL,
                                                                    SCENECOORDINATESYSTEM_WINDOW_PIXEL_TOP_LEFT,
                                                                    localScene, in_coords)
        if result == RESULT_OK:
    ...
    def unprojectLocal(self, x, y, z, localScene):
        ...
        result, out_coords = self._sceneviewer.transformCoordinates(SCENECOORDINATESYSTEM_WINDOW_PIXEL_TOP_LEFT,
                                                                    SCENECOORDINATESYSTEM_LOCAL, localScene, in_coords)
        if result == RESULT_OK:
            return out_coords  # [out_coords[0] / out_coords[3], out_coords[1] / out_coords[3], out_coords[2] / out_coords[3]]
    ...
    def selectNode(self, node):
        ...
        with ChangeManager(fieldmodule):
            ...
            nodesetGroup = selectionGroup.getOrCreateNodeGroup(nodeset)
        ...
        if (self._alignMode == self.AlignMode.NONE) and not self._editNode:
            ...
            if self._selectionKeyPressed:
                if button == QtCore.Qt.LeftButton:
                    ...
                    if node and (graphics.getType() == Graphics.TYPE_POINTS) and (
                            graphics.getFieldDomainType() == Field.DOMAIN_TYPE_NODES):
            ...
            if self._model and self._alignKeyPressed:
                ...
                if (button == QtCore.Qt.MiddleButton) or (
                        (button == QtCore.Qt.LeftButton) and (event.modifiers() & QtCore.Qt.SHIFT)):
                ...
                if result == RESULT_OK:
                    ...
                    if editVectorField.isValid() and (vectorField.getNumberOfComponents() == componentsCount) \
                        ...
                        if vectorField.getCoordinateSystemType() != Field.COORDINATE_SYSTEM_TYPE_RECTANGULAR_CARTESIAN:
                            editVectorField = fieldmodule.createFieldCoordinateTransformation(vectorField,
                                                                                              coordinateField)
                        ...
                        initialTipCoordinates = [(initialCoordinates[c] + initialVector[c] * pointScaleFactor) for c in
                                                 range(3)]
                        windowCoordinates = self.projectLocal(initialTipCoordinates[0], initialTipCoordinates[1],
                                                              initialTipCoordinates[2], localScene)
                        finalTipCoordinates = self.unprojectLocal(mousePos[0], -mousePos[1], windowCoordinates[2],
                                                                  localScene)
                        finalVector = [(finalTipCoordinates[c] - initialCoordinates[c]) / pointScaleFactor for c in
                                       range(3)]
                        result = editVectorField.assignReal(fieldcache, finalVector)
                    else:
                        windowCoordinates = self.projectLocal(initialCoordinates[0], initialCoordinates[1],
                                                              initialCoordinates[2], localScene)
                        xa = self.unprojectLocal(self._lastMousePos[0], -self._lastMousePos[1], windowCoordinates[2],
                                                 localScene)
